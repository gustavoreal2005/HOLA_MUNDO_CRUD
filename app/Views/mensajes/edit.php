...
<section>
  ...
  <form method="post" action="<?= (BASE_URL ? rtrim(BASE_URL,'/') : '') ?>/mensajes/update" enctype="multipart/form-data" class="form">
    <input type="hidden" name="id_mensaje" value="<?= (int)$mensaje['id_mensaje'] ?>"/>
    <label>Título
    ...
    <a class="btn secondary" href="<?= (BASE_URL ? rtrim(BASE_URL,'/') : '') ?>/mensajes/show?id=<?= (int)$mensaje['id_mensaje'] ?>">Cancelar</a>
  </form>
</section>
